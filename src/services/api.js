...
const API_BASE_URL = "http://inventra.runasp.net"
...
function mapPurchaseInvoices(apiInvoices) {
  return Array.isArray(apiInvoices)
    ? apiInvoices.map(mapPurchaseInvoice)
    : []
}
...
export const dashboardService = {
  async getStats() {
    ...
    return result.success ? mapDashboardStats(result.data) : mapDashboardStats(null)
  },
...
export const categoryService = {
  ...
  async getAllSubcategories(categoryId) {
    const result = await apiFetch(`/api/Categories/${categoryId}/subcategories`)
...
export const stockService = {
  ...
  async getByProductId(productId) {
    const result = await apiFetch(`/api/warehouse/stock/by-product/${productId}`)
    return result.success ? mapStockItems(result.data) : []
  },

  async getByLocationId(locationId) {
    const result = await apiFetch(`/api/warehouse/stock/by-location/${locationId}`)
    return result.success ? mapStockItems(result.data) : []
  },
...
export const paymentService = {
  ...
  async getBySupplierId(supplierId) {
    const result = await apiFetch(`/api/SupplierPayments/supplier/${supplierId}`)
    return result.success ? result.data : []
  },
