    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: 'Total Value', value: `$${invoices.reduce((s, i) => s + i.grandTotal, 0).toLocaleString('en', { minimumFractionDigits: 2 })}`, color: 'text-[#164E63]' },
        ].map(c => (
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex gap-3 flex-wrap">
        ...
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          ...
          <option>All Status</option><option>Paid</option><option>Pending</option><option>Cancelled</option>
        </select>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          ...
          <span className="text-sm text-slate-400">
            Showing {filtered.length} · Total: <span className="font-semibold text-slate-700">${totalValue.toFixed(2)}</span>
          </span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                {['INVOICE ID', 'SUPPLIER', 'DATE', 'DELIVERY DATE', 'SUBTOTAL', 'DISCOUNT', 'TAX', 'GRAND TOTAL', 'STATUS', 'ACTIONS'].map(h => (
                  <th key={h} className="text-xs font-semibold text-slate-500 uppercase tracking-wider text-left px-5 py-3">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={10}><EmptyState title="No invoices found" icon={FileText} /></td></tr>
              )}
              {filtered.map(inv => (
                <tr key={inv.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="px-5 py-3.5 text-sm font-mono text-slate-600 whitespace-nowrap">{inv.id}</td>
                  <td className="px-5 py-3.5 text-sm font-semibold text-slate-800 whitespace-nowrap">{inv.supplier}</td>
                  <td className="px-5 py-3.5 text-sm text-slate-600 whitespace-nowrap">{inv.date}</td>
                  <td className="px-5 py-3.5 text-sm text-slate-600 whitespace-nowrap">{inv.deliveryDate}</td>
                  <td className="px-5 py-3.5 text-sm text-slate-700">${inv.total?.toFixed(2)}</td>
                  <td className="px-5 py-3.5 text-sm text-green-600">-${inv.discount?.toFixed(2)}</td>
                  <td className="px-5 py-3.5 text-sm text-slate-600">${inv.tax?.toFixed(2)}</td>
                  <td className="px-5 py-3.5 text-sm font-semibold text-slate-800">${inv.grandTotal?.toFixed(2)}</td>
      ...
      <Modal open={!!viewModal} onClose={() => setViewModal(null)} title="Purchase Invoice Details">
        <InvoiceDetailModal invoice={viewModal} onClose={() => setViewModal(null)} />
      </Modal>
    </div>
  )
}
