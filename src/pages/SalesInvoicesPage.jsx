...
import { PageLoader, EmptyState, ConfirmModal } from '../components/ui/Feedback'
...
function PaymentBadge({ payment }) {
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold ${
      payment === 'CASH' ? 'bg-green-50 text-green-700' : 'bg-blue-50 text-blue-700'
    }`}>
      {payment}
    </span>
  )
    ...
    <div className="space-y-5">
      ...
      <div className="border-t border-slate-100 pt-4 space-y-2 text-sm">
        ...
        <div className="flex justify-between border-t border-slate-100 pt-2">
          ...
          <span className="font-bold text-lg text-[#164E63]">${invoice.grandTotal?.toFixed(2)}</span>
        </div>
      </div>
    </div>
  )
...
export default function SalesInvoicesPage() {
  ...
  const handleAction = async () => {
    ...
    try {
      if (confirmAction.type === 'refund') await salesService.processRefund(confirmAction.id)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Refunded', value: invoices.filter(i => i.status === 'Refunded').length, color: 'text-amber-600' },
          { label: 'Revenue', value: `$${totalRevenue.toLocaleString('en', { minimumFractionDigits: 2 })}`, color: 'text-[#164E63]' },
        ].map(c => (
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm">
        <div className="px-5 py-4 border-b border-slate-100">
          <h2 className="text-base font-semibold text-slate-800">Sales Ledger</h2>
        </div>
        ...
        <div className="w-full max-w-full overflow-x-auto">
          <table className="w-full min-w-[900px]">
            <thead className="bg-slate-50">
              <tr>
                {['INVOICE ID', 'CUSTOMER', 'CASHIER', 'DATE', 'TOTAL', 'DISCOUNT', 'TAX', 'GRAND TOTAL', 'PAYMENT', 'STATUS', 'ACTIONS'].map(h => (
                  <th key={h} className="text-xs font-semibold text-slate-500 uppercase tracking-wider text-left px-3 md:px-5 py-3">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              ...
              {filtered.map(inv => (
                <tr key={inv.id} className="border-t border-slate-100 hover:bg-slate-50">
                  <td className="px-3 md:px-5 py-3.5 text-sm font-mono text-slate-600">{inv.id}</td>
                  <td className="px-3 md:px-5 py-3.5">
                    <p className="text-sm font-semibold text-slate-800">{inv.customer}</p>
                    <p className="text-xs text-slate-400">{inv.customerNumber}</p>
                  </td>
                  <td className="px-3 md:px-5 py-3.5 text-sm text-slate-600">{inv.cashier}</td>
                  <td className="px-3 md:px-5 py-3.5 text-sm text-slate-600">{inv.date}</td>
                  <td className="px-3 md:px-5 py-3.5 text-sm text-slate-700">${inv.total?.toFixed(2)}</td>
                  <td className="px-3 md:px-5 py-3.5 text-sm text-green-600">-${inv.discount?.toFixed(2)}</td>
                  <td className="px-3 md:px-5 py-3.5 text-sm text-slate-600">${inv.tax?.toFixed(2)}</td>
                  <td className="px-3 md:px-5 py-3.5 text-sm font-semibold text-slate-800">${inv.grandTotal?.toFixed(2)}</td>
                  <td className="px-3 md:px-5 py-3.5">
                  ...
                  <td className="px-3 md:px-5 py-3.5">
                    <div className="flex items-center gap-1">
                      <button onClick={() => setViewModal(inv)} className="text-slate-400 hover:text-[#164E63] p-1">
                        <Eye size={15} />
                      </button>

                      {inv.status === 'Completed' && (
                        <>
                          <button onClick={() => setConfirmAction({ type: 'refund', id: inv.id, label: inv.id })} className="text-slate-400 hover:text-amber-500 p-1">
                            <RotateCcw size={15} />
                          </button>
                          <button onClick={() => setConfirmAction({ type: 'void', id: inv.id, label: inv.id })} className="text-slate-400 hover:text-red-500 p-1">
                            <XCircle size={15} />
                          </button>
                        </>
                      )}
      ...
      <Modal open={!!viewModal} onClose={() => setViewModal(null)} title="Sales Invoice Details">
      ...
      <ConfirmModal
        ...
        title={confirmAction?.type === 'refund' ? 'Process Refund' : 'Void Invoice'}
